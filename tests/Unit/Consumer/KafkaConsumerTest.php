...
{
    public function testConstructorWithKafkaConnection(): void
    {
        $connection = $this->createMock(KafkaConnection::class);
        $consumer = new KafkaConsumer($connection, 'test-group');

        $this->assertInstanceOf(KafkaConsumer::class, $consumer);
    }

    public function testConstructorWithExternalKafkaConnection(): void
    {
        $connection = $this->createMock(ExternalKafkaConnection::class);
        $consumer = new KafkaConsumer($connection, 'test-group');
    ...
    public function testConstructorWithCustomConfig(): void
    {
        ...
        $consumer = new KafkaConsumer($connection, 'test-group', $customConfig);

        $this->assertInstanceOf(KafkaConsumer::class, $consumer);
    }

    public function testInitializeConsumerThrowsExceptionWhenExternalConnectionHasProducer(): void
    {
        $mockProducer = $this->createMock(Producer::class);
        $mockConnection = $this->createMock(ExternalKafkaConnection::class);
        $mockConnection->method('getClient')->willReturn($mockProducer);

        $consumer = new KafkaConsumer($mockConnection, 'test-group');

        $this->expectException(ConsumerException::class);
        $this->expectExceptionMessage('ExternalKafkaConnection for consumer must wrap a RdKafka\KafkaConsumer instance');

        // Trigger initialization via reflection
        $reflection = new \ReflectionClass($consumer);
        $method = $reflection->getMethod('initializeConsumer');
